...
class ModalityDiscriminator(nn.Module):
    ...
    def _reshape_input(self, x):
        if x.dim() == 2:
            if x.size(-1) != self.input_dim:
                raise ValueError('input feature dim mismatch')
            return x
        if x.dim() == 3:
            if x.size(-1) != self.input_dim:
                raise ValueError('input feature dim mismatch')
    ...
    def forward(self, x):
        ...
        x_adv = GradientReversalFunction.apply(x_flat, self.alpha)
        return self.net(x_adv)
...
class HeterGConv_Edge(torch.nn.Module):
    ...
    def __init__(self,
        ...
        base_dropout = min(max(float(dropout), 0.1), 0.2)
        self.hetergcn_layers = _get_clones(encoder_layer, num_layers)
        self.fc_layers = _get_clones(
            nn.Sequential(nn.Linear(feature_size, feature_size),
                          nn.LeakyReLU(), nn.Dropout(base_dropout)),
            num_layers)
    ...
    def _flatten_speaker_ids(self, qmask, dia_lens):
        if qmask is None:
            return None
        all_speakers = []
        batch_size = qmask.size(1)
        for b in range(batch_size):
            cur_len = int(dia_lens[b])
            q = qmask[:cur_len, b]
            if q.dim() == 2:
                sp = torch.argmax(q, dim=-1)
            else:
                sp = q.long()
            all_speakers.append(sp)
        if len(all_speakers) == 0:
            return None
        return torch.cat(all_speakers, dim=0).detach().cpu().tolist()
    ...
    def _heter_no_weight_edge(self, feature, num_modal, dia_lens, win_p, win_f,
        ...
        speaker_ids = self._flatten_speaker_ids(qmask, dia_lens)
        ...
        for dia_len in dia_lens:
            ...
            if speaker_ids is not None:
                local_speakers = speaker_ids[start:start + dia_len]
                for m in range(num_modal):
                    local_nodes = nodes_uni[m][start:start + dia_len]
                    for i in range(dia_len):
                        for j in range(i + 1, dia_len):
                            if local_speakers[i] == local_speakers[j]:
                                ni = local_nodes[i]
                                nj = local_nodes[j]
                                index_inter.append((ni, nj))
                                index_inter.append((nj, ni))
                                index_inter.append((ni, nj))
                                index_inter.append((nj, ni))
            start += dia_len
